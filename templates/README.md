# Premium Template Format

This directory contains premium templates in markdown format that get seeded into the database.

## Template Structure

Each template is a `.md` file with YAML frontmatter + detailed markdown specification.
...
previewVideoUrl: "https://d8j0ntlcm91z4.cloudfront.net/..."
...
1. **Create markdown file** in `/templates/` directory
2. **Add frontmatter** with all metadata
3. **Write detailed specification** in markdown
4. **Run seeding script**: `pnpm db:seed-templates`

## Template Categories

- `hero` - Hero sections
- `landing` - Full landing pages
...
- `starter` - Starter plan required
- `pro` - Pro plan required
- `team` - Team plan required

## Best Practices

1. **Be extremely specific** - Include exact class names, pixel values, HSL colors
2. **Include all assets** - Fonts, videos, images with full URLs
3. **Specify animations** - Full keyframe definitions
4. **List dependencies** - Exact package names and versions
5. **Add complexity metadata** - Helps users filter templates
6. **Include responsive specs** - Mobile, tablet, desktop breakpoints
7. **Provide context** - Explain design decisions and usage
